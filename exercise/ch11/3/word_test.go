...
func randomPalindrome(rng *rand.Rand) string {
	...
	for i := 0; i < count; i++ {
		var r rune
		if i == j {
			r = rune(punctuations[j%len(punctuations)])
		} else {
...
func randomNonPalindorome(rng *rand.Rand) string {
	per := "abcd"
	tail := "efgi"
	n := rng.Intn(25)
	runes := make([]rune, n)
