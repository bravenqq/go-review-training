...
const (
	...
	Int8Max  int8  = int8(^uint8(0) >> 1) //此处一定要使用无符号的0，使用int8(0),^int8(0)因为有符号整数是补码形式保存的，得到值11111111在计算机中为负数，仍然是int8类型，然后右移一位，负数移位符号位会保留，得到还是11111111，是补码形式，对应的原码是10000001值为-1
